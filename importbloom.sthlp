...
{p2colset 1 21 18 2}{...}
{p2col:{bf: importbloom} {hline 2}}Import Bloomberg data{p_end}
{p2colreset}{...}
...
{p 8 32 2}
{cmd:importbloom} {cmd:using} {it:{help filename}}
    [{cmd:,}
      {it:{help importbloom##importbloom_options:importbloom options}}]


{synoptset 37}{...}
{marker importbloom_options}{...}
{synopthdr :importbloom options}
{synoptline}
...
{pstd}
{cmd:importbloom} loads an Excel file containing data exported by bloomberg into Stata.

{marker importoptions}{...}
{title:Options for importbloom}
...
{pstd}
        ...
        {cmd:. importbloom using "data/Vantaggio competitivo e WACC.xlsx", cellrange(A4) sheet("Foglio1") datastart(B) nvar(32) lasttick(DEP)}

{pstd}

        {cmd:. importbloom using "data/us_banks.xlsx", cellrange(A4) sheet("US1") datastart(B) nvar(13) lasttick(GQO) }
